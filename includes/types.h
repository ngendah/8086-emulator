...
typedef struct _null_t final {
  friend std::ostream &operator<<(std::ostream &os, const _null_t &) {
    os << " null_t{} ";
    return os;
  }
} null_t;

typedef struct _opcode_t final {
  uint8_t OPCODE;
  friend std::ostream &operator<<(std::ostream &os, const _opcode_t &t) {
    os << " opcode_t{" << fmt::format("OPCODE=0x{:x}(0b{:b})", t.OPCODE)
       << "} ";
    return os;
  }
} opcode_t;

typedef struct _opcode_d_w_t final {
  uint8_t W : 1;
  uint8_t D : 1;
  uint8_t OPCODE : 6;
  operator uint8_t() const { return (uint8_t)(OPCODE << 2 | D << 1 | W); }
  friend std::ostream &operator<<(std::ostream &os, const _opcode_d_w_t &t) {
    os << " opcode_d_w_t{"
       << fmt::format("OPCODE=0x{0:x}(0b{0:b}), D=0x{1:x}(0b{1:b}), "
                      "W=0x{2:x}(0b{2:b})",
                      t.OPCODE, t.D, t.W)
       << "} ";
    return os;
...
typedef struct _mod_reg_rm_t final {
  ...
  uint8_t REG : 3;
  uint8_t MOD : 2;
  operator uint8_t() const { return (uint8_t)(MOD << 6 | REG << 3 | RM); }
...
typedef struct _mod_sr_rm_t final {
  ...
  operator uint8_t() const {
    assert(_X == 0);
    return (uint8_t)(MOD << 6 | 0 << 5 | SR << 3 | RM);
  }
  friend std::ostream &operator<<(std::ostream &os, const _mod_sr_rm_t &t) {
    os << " mod_sr_rm_t{"
       << fmt::format("MOD=0x{0:x}(0b{0:b}), _X=0x{1:x}(0b{1:b}), "
                      "SR=0x{2:x}(0b{2:b}), RM=0x{3:x}(0b{3:b})",
                      t.MOD, t._X, t.SR, t.RM)
       << "} ";
    return os;
  }
} mod_sr_rm_t;
...
typedef struct _w_reg_t final {
  uint8_t REG : 3;
  uint8_t W : 1;
  uint8_t _X : 4;
  operator uint8_t() const {
    assert(_X == 0xB);
    return (uint8_t)(0xB << 4 | W << 3 | REG);
  }
  friend std::ostream &operator<<(std::ostream &os, const _w_reg_t &t) {
    os << " w_reg_t{"
       << fmt::format("_X=0x{0:x}(0b{0:b}), W=0x{1:x}(0b{1:b}), "
                      "REG=0x{2:x}(0b{2:b})",
                      t._X, t.W, t.REG)
       << "} ";
    return os;
...
typedef struct _d_w_t final {
  ...
  operator uint8_t() const {
    assert(_X == 0x28);
    return (uint8_t)(0x28 << 2 | D << 1 | W);
  }
  friend std::ostream &operator<<(std::ostream &os, const _d_w_t &t) {
    os << " d_w_t{"
       << fmt::format("_X=0x{0:x}(0b{0:b}), D=0x{1:x}(0b{1:b}), "
                      "W=0x{2:x}(0b{2:b})",
                      t._X, t.D, t.W)
       << "} ";
    return os;
