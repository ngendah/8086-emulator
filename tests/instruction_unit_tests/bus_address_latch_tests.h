...
TEST(AddressLatchTests, test_latch) {
  ...
  RAM ram(&_buffer.at(0), 125);
  auto bus = BUS::from_device(&ram);
  {
    AddressLatch busio(&bus, Address((uint16_t)45));
  ...
  {
    AddressLatch busio(&bus, Address((uint16_t)25));
  ...
  {
    AddressLatch busio(&bus, Address((uint16_t)10));
    busio.to_u8().write(0xAD);
  }
  {
    AddressLatch busio(&bus, Address((uint16_t)11));
  ...
  {
    AddressLatch busio(&bus, Address((uint16_t)35));
